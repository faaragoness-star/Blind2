...
if (!defined('G3D_FLOW_OPT_EMBEDS')) define('G3D_FLOW_OPT_EMBEDS', 'g3dflow_embeds');
if (!defined('G3D_FLOW_SIG_OPTION')) define('G3D_FLOW_SIG_OPTION', 'g3d_sig_key');
...
function shortcode_viewer($atts){
  $a = shortcode_atts([
  ...
  <div id="g3d-viewer" class="g3d-viewer">
    <iframe src="<?php echo esc_url($a['viewer_url']); ?>" style="width:100%;aspect-ratio:16/9;border:0" title="G3D Viewer" loading="lazy"></iframe>
  </div>
...
function admin_menu(){
  add_menu_page(
    'G3D Flow', 'G3D Flow', 'manage_options', 'g3d-flow',
    __NAMESPACE__.'\render_admin', 'dashicons-visibility', 59
  );
}

function render_admin(){
  $sig = get_option(G3D_FLOW_SIG_OPTION, 'dev-key');
...
function api_validate_sign(\WP_REST_Request $request){
  ...
  $ttl = 3600; // 1h
  $expires = time() + $ttl;
  $sigKey = get_option(G3D_FLOW_SIG_OPTION, 'dev-key');
...
function api_verify(\WP_REST_Request $request){
  $data = $request->get_json_params();
  $hash = isset($data['sku_hash']) ? (string)$data['sku_hash'] : '';
  $signature = isset($data['signature']) ? (string)$data['signature'] : '';
  $expires_at = isset($data['expires_at']) ? strtotime($data['expires_at']) : 0;
  $now = time();
  ...
  $provided = isset($parts[2]) ? base64_decode($parts[2]) : '';
